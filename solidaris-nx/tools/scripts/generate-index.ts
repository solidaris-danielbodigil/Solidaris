...
function findComponents(dir: string): Map<string, string> {
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      const subDirs = fs.readdirSync(componentDir, { withFileTypes: true }).filter((d) => d.isDirectory());
...
function hasMetadata(componentPath: string): boolean {
  ...
  return fs.readdirSync(dir).some((f) => f.endsWith('.metadata.ts'));
}

function detectBemBlock(componentPath: string): string | null {
  const scssPath = componentPath.replace('.component.ts', '.component.scss');
  if (!fs.existsSync(scssPath)) return null;
  const content = fs.readFileSync(scssPath, 'utf-8');
  const match = content.match(/\.(c-[a-z][a-z0-9-]*|sds-[a-z][a-z0-9-]*)\s*\{/);
...
function findScssPath(componentName: string): string | null {
  const kebab = componentName
    .replace(/([a-z])([A-Z])/g, '$1-$2')
    .toLowerCase();
  const scssFile = path.join(WORKSPACE_ROOT, `libs/styles/src/06-components/_${kebab}.scss`);
  return fs.existsSync(scssFile) ? `libs/styles/src/06-components/_${kebab}.scss` : null;
}
...
function generate(): void {
  ...
  const output: IndexFile = {
    ...
    tokenArchitecture: existingIndex?.tokenArchitecture ?? {
      prefix: '--sds-*',
      prefixConfig: 'libs/styles/src/01-settings/_prefix.scss',
      layers: {
        primitive: {
          files: [
            'libs/styles/src/01-settings/_colors-primitive.scss',
            'libs/styles/src/01-settings/_typography-primitive.scss',
          ],
          pattern: '--sds-{category}-{palette}-{shade}',
        },
        semantic: {
          files: [
            'libs/styles/src/01-settings/_colors-semantic.scss',
            'libs/styles/src/01-settings/_typography-semantic.scss',
            'libs/styles/src/01-settings/_spacing.scss',
            'libs/styles/src/01-settings/_radius.scss',
            'libs/styles/src/01-settings/_shadows.scss',
            'libs/styles/src/01-settings/_transitions.scss',
            'libs/styles/src/01-settings/_focus.scss',
            'libs/styles/src/01-settings/_globals.scss',
          ],
          pattern: '--sds-{category}-{role}',
        },
        component: {
          files: ['libs/styles/src/06-components/*.scss'],
